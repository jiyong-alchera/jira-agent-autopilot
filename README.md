...
| `run-epic-loop.js` | **연속 개발** — 한 상위 카드(에픽·워크스트림)의 미완료 하위 태스크를 생성순으로 하나씩 개발→PR→리뷰 승인→병합 대기 |
...
- **카드 상태**: 단계(plan대기/답변대기/build대기/실패/완료) 표. 행을 펼치면 원문·코멘트 열람, **답변(인용 답글) 작성 + claude-answered 라벨**, **이 카드만 즉시 실행**, **엔진 실행 로그**(도구 호출/메시지/결과, 3초 갱신) 확인.
  - build 실행 시 **`🔁 승인까지`** 를 켜면 개발 → PR 생성 후 **그 PR 이 리뷰 승인될 때까지 '리뷰 → 반영 → 재리뷰'** 를 자동 반복한다(최대 5회, 회차마다 Slack 알림, 'PR 목록'에서 중지 가능).
- **에픽 연속 개발**: 상위 카드 + 대상 repo 를 고르면 그 **미완료 하위 태스크를 생성순으로 하나씩** 자동 처리한다 —
  라벨 부여 → plan(질문) → **plan 의 제안 답변 자동 채택** → 개발·PR → **승인까지 리뷰 루프** → **사용자가 PR 을 모두 병합하면 다음 태스크**.
  상위 카드 본문(설계안)이 모든 하위 태스크의 프롬프트에 주입돼 설계 방향이 일관되게 유지된다.
  대상은 **에픽 계층 카드면 무엇이든** 되며, 그 계층 이름은 프로젝트를 따른다 — EKYB·FSIF 는 '에픽', PHYS 는 '워크스트림'.
  사람의 필수 개입은 **PR 병합 하나**이며, 어느 단계든 실패하면 멈추고 알림을 보낸 뒤 **[이어서 진행]** 으로 그 지점부터 재개한다.
  패널 안에서 **진행 로그(`loop-epic.log`)와 현재 태스크의 엔진 실행 로그**를 3초 간격으로 볼 수 있고, **PR 목록에서 개별·선택·전체 병합**을 바로 할 수 있다.
  **'리뷰 승인 후 자동 병합'**(기본 꺼짐)을 켜면 승인된 PR 을 지정 시간(기본 60분) 뒤 자동 병합해 완전 무인으로 돌릴 수도 있다.
  **'중단 시 자동 재시도'**(기본 꺼짐)를 켜면 토큰(사용량 한도) 소진처럼 시간이 지나면 풀리는 중단을 해제 시각에 맞춰 자동 재개한다.
  **'base 충돌 시 자동 해소·재푸시'**(기본 꺼짐)를 켜면 병합 대기 중 충돌난 PR 을 지정 시간(기본 15분) 뒤 rebase 로 해소·검증·force-push 하고 승인 무효화 후 재리뷰까지 이어간다 — 대시보드·Slack 버튼으로 즉시 시킬 수도 있다.
- **처리 이력**: 처리한 카드/시각/결과/PR·브랜치 기록 표.
- **실시간 로그**: `loop-plan/build/review/epic.log` tail + 비우기. 모든 섹션은 접기/펼치기, 핫리로드(`npm run dev`) 지원.
