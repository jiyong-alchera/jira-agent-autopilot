...
| `run-epic-loop.js` | **에픽 연속 개발** — 한 에픽의 미완료 하위 태스크를 생성순으로 하나씩 개발→PR→리뷰 승인→병합 대기 |
...
- **에픽 연속 개발**: 에픽 + 대상 repo 를 고르면 그 에픽의 **미완료 하위 태스크를 생성순으로 하나씩** 자동 처리한다 —
  라벨 부여 → plan(질문) → **plan 의 제안 답변 자동 채택** → 개발·PR → **승인까지 리뷰 루프** → **사용자가 PR 을 모두 병합하면 다음 태스크**.
  에픽 본문(설계안)이 모든 하위 태스크의 프롬프트에 주입돼 설계 방향이 일관되게 유지된다.
