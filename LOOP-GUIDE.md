...
- **리뷰 승인 알림은 CI 가 끝난 뒤에 옵니다.** 승인은 PR 을 올린 직후 끝나는데 그때는 CI 가 아직 도는 중이라,
  그 시점에 온 `[병합]` 버튼은 눌러도 막혔습니다(그래서 CI 후 알림이 또 왔습니다). 이제 CI 가 초록이 된 뒤
  `✅ … CI 통과` 알림 하나만 오고, 그 버튼은 바로 눌립니다. CI 가 빨갛거나 미확정이면 병합 버튼 없이 사유만 알립니다.
- **CI 가 빨간 PR 은 자동 병합되지 않습니다.** 승인과 대기 시간을 채웠더라도 막습니다.
  대시보드에서 사람이 직접 병합할 때는 확인창에 CI 상태를 알려주고, 그래도 진행을 고르면 넘어갑니다.
- **base 충돌도 스스로 해소합니다.** 병합 대기 중 다른 사람의 병합으로 충돌(`⚠️ 충돌`)이 나면 알림을 보내고,
  **'base 충돌 시 자동 해소·재푸시'** 를 켜두면 지정한 시간(기본 15분) 뒤 `rebase 로 해소 → 테스트/빌드 검증 →
  force-push` 까지 대신 합니다. 해소한 코드도 아무도 안 본 코드라 **승인을 무효화하고 재리뷰**를 받은 뒤 병합으로 갑니다.
  안전하게 해소할 수 없으면 푸시하지 않고 멈춰서 알립니다. 꺼져 있어도 PR 목록의 **[충돌 해소·재푸시]** 버튼이나
  **Slack 알림의 같은 버튼**으로 즉시 시킬 수 있습니다.
