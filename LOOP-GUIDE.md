...
| `run-epic-loop.js` | 에픽 하나의 미완료 하위 태스크를 **생성순으로 하나씩** 끝까지 처리 (대시보드 '에픽 연속 개발') |
...
## 에픽 단위로 한 번에 (에픽 연속 개발)

카드를 하나씩 챙기는 대신, **에픽의 하위 태스크 전체**를 순서대로 맡길 수 있습니다.
대시보드 **'에픽 연속 개발'** 패널에서 프로젝트·에픽·대상 repo 를 고르고 시작하면:
...
- 에픽 본문(설계안)이 모든 하위 태스크의 plan/build 프롬프트에 주입됩니다.
...
- **'중단 시 자동 재시도'** 를 켜면, 토큰(사용량 한도) 소진처럼 시간이 지나면 풀리는 이유로 멈췄을 때
  ...
  사람이 봐야 하는 중단(제안 답변 없음·리뷰 미승인·자동 병합 실패)은 재시도하지 않습니다.
...
- 아직 차례가 아닌 하위 카드에는 트리거 라벨을 붙이지 않으므로 plan/build 루프가 순서를 앞지르지 않습니다.
  다만 **이미 `claude-work` 가 붙어 있던 카드**는 루프도 볼 수 있으니, 에픽 실행 중에는 두 루프를 멈춰두길 권합니다.
