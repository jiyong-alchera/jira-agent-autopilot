...
[이슈 첨부 문서] 아래 파일들은 이 Jira 이슈(${ISSUE_KEY})에 첨부된 문서(PDF·텍스트·스펙·코드 등)입니다. '.docx.txt'·'.xlsx.txt'·'.pptx.txt' 는 오피스 첨부에서 시스템이 추출한 텍스트입니다(원본 서식·이미지는 빠져 있습니다). 작업을 시작하기 전에 반드시 'Read' 도구로 각 파일을 열어 내용을 파악하고, 요구사항 이해와 구현에 반영하세요:
${_docs}"
fi
...
- 구현 전에 명확히 해야 할 질문들을 정리해, Jira 이슈 ${ISSUE_KEY} 에 코멘트로 작성하세요.
  ...
  각 질문 바로 아래 줄에, 코드베이스를 살펴본 결과에 근거한 '제안 답변'을 반드시 한 줄 덧붙이세요.
  형식은 다음을 정확히 지키세요(대시보드가 이 줄을 파싱해 답변란을 채웁니다):
     💡 제안: <제안하는 답변> (근거: <한 줄 근거>)
  제안은 담당자가 '그대로 가자'만 판단하면 되도록 구체적이고 실행 가능한 결론이어야 합니다.
  근거는 코드베이스에서 확인한 사실을 우선 쓰고, 확인하지 못해 추측이면 '추측'이라고 밝히세요.
