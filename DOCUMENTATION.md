...
- **상태 파일**: `.state/<EPIC>.epic.json` — `{status, reason, step, index, total, current:{key,summary,step}, tasks[], repos[]}`.
  대시보드가 5초마다 폴링해 진행률·현재 단계·중단 사유를 표시한다.
